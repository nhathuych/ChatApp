...
export default class Autoclick extends Controller {
  ...
  appear(entry) {
    ...
    if (!Autoclick.throttling) {
      ...
      Autoclick.messagesContainer = document.getElementById('js-messages-container')
  ...
  static throttle(func, wait) {
    ...
    return function() {
      let now = Date.now()
      let reminning = wait - (now - previous)

      if (reminning <= 0 || reminning > wait) {
        if (timeout) {
          clearTimeout(timeout)
        }
        later()
      } else if (!timeout) {
        timeout = setTimeout(later, reminning)
      }
    }
  }
}
